...
const vehicleModelWithConfigsInclude = {
  vehicleConfigs: {
...
const computeVehicleWeightMetrics = (vehicle) => {
  ...
  const packageConfigIds = getPackageConfigIds(vehicle.optionPackages);
  const configWeight = vehicle.vehicleConfigs.reduce((sum, vc) => {
    if (packageConfigIds.has(vc.configItemId)) {
      return sum;
    }
    const weight = vc.actualWeight || vc.configItem.typicalWeight;
...
const fetchVehiclesWithConfigs = async (where = {}) => {
  return prisma.vehicleModel.findMany({
    where,
    include: vehicleModelWithConfigsInclude,
  });
};

module.exports = {
  vehicleModelWithConfigsInclude,
  getPackageConfigIds,
